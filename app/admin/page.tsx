...
export default function AdminDashboard() {
  ...
  const handleFixTimestamps = async (e: React.FormEvent) => {
    e.preventDefault()
    const weekInput = (e.target as any).fix_week_input.value
    const weekNum = parseInt(weekInput)
    ...
    try {
      const res = await fetch('/api/admin/fix-week-timestamps', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ week: weekNum }),
      })
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Failed to fix timestamps')
      }
    } catch (err) {
      setError('Fix failed')
      console.error(err)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="bg-cyan-50 border border-cyan-200 p-6 rounded">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-cyan-200 pt-3">
                <p className="text-xs font-semibold text-cyan-700 mb-2">🔧 Fix Week Timestamps</p>
                <form onSubmit={handleFixTimestamps} className="space-y-2">
                  <input
                    type="number"
                    name="fix_week_input"
                    placeholder="Week #"
